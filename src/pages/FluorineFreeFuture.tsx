...
export default function FluorineFreeFuture() {
  ...
  return (
    <PageShell>
      ...
      <ScrollSpySections idPrefix="technology" label="技术目录" showIndex={false} compactNav items={sections.map((section) => ({ id: section.section_key || String(section.id), label: section.nav_label || section.title, content: <TechnologyDetail section={section} /> }))} />
    </PageShell>
  )
}
