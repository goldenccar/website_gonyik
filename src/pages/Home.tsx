      ...
      <PageSection tone="navy">
        ...
        <div className="grid items-stretch gap-x-6 gap-y-10 md:grid-cols-3">
          {['otter', 'rayo', 'kais'].map((slug) => series.find((item) => item.slug === slug)).filter(Boolean).map((item) => {
            return <Link key={item!.id} to={`/fabrics?series=${item!.slug}`} className="group relative flex h-full min-w-0 flex-col focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#69B2C1]">
              <div className="relative aspect-[5/3] overflow-hidden bg-dark after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-white/15">
                ...
                {item!.home_badge_image && <img src={item!.home_badge_image} alt={`${item!.name} 系列徽章`} loading="lazy" decoding="async" className="absolute bottom-5 left-5 z-10 w-[128px] object-contain drop-shadow-[0_3px_8px_rgba(0,0,0,0.28)] transition-transform duration-[var(--motion-media)] ease-apple group-hover:-translate-y-0.5 group-focus-visible:-translate-y-0.5 md:bottom-6 md:left-6 md:w-[148px]" />}
              </div>
              ...
              <span className="mt-auto pt-6 text-[12px] font-medium tracking-[0.02em] text-white/65 transition-colors duration-[var(--motion-instant)] group-hover:text-white group-focus-visible:text-white"><span className="relative inline-block after:absolute after:inset-x-0 after:-bottom-1.5 after:h-px after:origin-left after:scale-x-0 after:bg-[#69B2C1] after:transition-transform after:duration-[var(--motion-instant)] group-hover:after:scale-x-100 group-focus-visible:after:scale-x-100">查看系列 <span className="ml-0.5 inline-block transition-transform duration-[var(--motion-instant)] group-hover:translate-x-1 group-focus-visible:translate-x-1">→</span></span></span>
            </Link>
          })}
        </div>
      </PageSection>
