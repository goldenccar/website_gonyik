    ...
    <PageShell>
      ...
      <PageSection className="!py-8 lg:!py-10">
        ...
        {loadingProducts ? <div className="border-t border-border py-8 text-body text-secondary">正在加载该分类内容…</div> : <HorizontalRail label={`${category?.name || ''}应用`} mobileStack>
          ...
          {page?.rail_end_card_visible !== false && <article className="flex min-h-[180px] snap-start items-end bg-white p-7 sm:min-h-full"><div><p className="text-label text-secondary">IN DEVELOPMENT</p><h3 className="mt-3 text-h4 text-primary">{page?.rail_end_card_title || '新应用开发中'}</h3><p className="mt-3 text-body text-secondary">{page?.rail_end_card_description || '围绕新的任务与穿着环境持续开发。'}</p>{page?.rail_end_card_cta_label && <a href={page.rail_end_card_cta_href || '/contact'} className="mt-6 inline-block text-[14px] underline underline-offset-4">{page.rail_end_card_cta_label} →</a>}</div></article>}
        </HorizontalRail>}
        </div>
        </div>
      </PageSection>
