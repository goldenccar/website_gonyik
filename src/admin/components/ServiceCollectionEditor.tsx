...
export default function ServiceCollectionEditor({ moduleType }: { moduleType: ServiceModuleType }) {
  ...
  return <div className="bg-dark">
    ...
    {showForm && <Modal title={editing ? '编辑' : `新增${collection.itemLabel}`} onClose={() => setShowForm(false)}><form onSubmit={submit} className="space-y-4">{moduleType === 'care' ? <><FormField label="步骤标题" name="title" markup="inline" defaultValue={editing?.title} required /><FormField label="说明" name="content" markup="inline" defaultValue={editing?.content} textarea rows={4} required /></> : <><FormField label="问题" name="question" markup="inline" defaultValue={editing?.question} required /><FormField label="答案" name="answer" markup="inline" defaultValue={editing?.answer} textarea rows={4} required /></>}<SaveCancelButtons onCancel={() => setShowForm(false)} /></form></Modal>}
  </div>
}
