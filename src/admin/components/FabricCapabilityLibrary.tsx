...
export default function FabricCapabilityLibrary({ items, onChange, onMessage }: Props) {
  ...
  const remove = async (item: FabricCapabilityDefinition) => {
    ...
    const response = await api.delete(`/fabrics/admin/capabilities/${item.id}`)
    await onChange()
    const affected = Number(response.data?.affected || 0)
    onMessage(affected ? `性能标签已删除，已清空 ${affected} 张面料卡片中的对应字段` : '性能标签已删除')
  }
    ...
    <section className="mb-8 bg-dark p-4 sm:p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[12px] text-muted">统一管理面料卡片可选择的性能标签。</p>
        </div>
        <PrimaryButton onClick={() => { setEditing(null); setError(''); setOpen(true) }} icon={<Plus size={16} />}>新增标签</PrimaryButton>
      </div>
      <div className="flex flex-wrap gap-2">
        {items.map((item) => (
          <div key={item.key} className="flex h-10 items-center border border-white/15 pl-3 text-accent">
