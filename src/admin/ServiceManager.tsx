...
export default function AdminServiceManager() {
  ...
  return <Dashboard>
    ...
    {sectionDraft && <Modal title="编辑页面模块" onClose={() => setSectionDraft(null)}><form onSubmit={saveSection} className="space-y-4"><FormField label="目录名称" name="nav_label" required markup="inline" value={sectionDraft.nav_label || ''} onChange={(event) => setSectionDraft({ ...sectionDraft, nav_label: event.target.value })} /><FormField label="英文标签" name="eyebrow" markup="inline" value={sectionDraft.eyebrow || ''} onChange={(event) => setSectionDraft({ ...sectionDraft, eyebrow: event.target.value })} /><FormField label="章节标题" name="title" required markup="inline" value={sectionDraft.title} onChange={(event) => setSectionDraft({ ...sectionDraft, title: event.target.value })} /><FormField label="章节说明" name="subtitle" textarea rows={3} markup="inline" value={sectionDraft.subtitle || ''} onChange={(event) => setSectionDraft({ ...sectionDraft, subtitle: event.target.value })} /><SaveCancelButtons onCancel={() => setSectionDraft(null)} /></form></Modal>}
  </Dashboard>
}
