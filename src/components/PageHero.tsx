    ...
    <section className={`overflow-hidden bg-bg px-4 md:px-6 ${className}`}>
      <div className={`relative isolate mx-auto flex w-full max-w-[1760px] items-center overflow-hidden bg-dark ${height}`}>
        ...
        <div className={`w-full max-w-[720px] px-7 text-white md:px-12 lg:px-16 ${home ? 'py-16' : 'py-12'}`}>
          {tag && <p style={home ? { '--motion-delay': '70ms' } as CSSProperties : undefined} className={`label-en text-white/80 ${home ? 'motion-hero-item mb-5' : 'mb-4'}`}><InlineMarkup text={tag} /></p>}
          <h1 style={home ? { '--motion-delay': '140ms' } as CSSProperties : undefined} className={`whitespace-pre-line ${headingLanguageClass(title)} ${home ? 'motion-hero-item type-hero' : 'type-page-title'}`}><InlineMarkup text={title} /></h1>
          {subtitle && <p style={home ? { '--motion-delay': '220ms' } as CSSProperties : undefined} className={`body-copy mt-5 max-w-[600px] text-white/85 ${home ? 'motion-hero-item' : ''}`}><InlineMarkup text={subtitle} /></p>}
