    ...
    <section className={`page-intro page-intro--${variant} ${className}`}>
      <div className="page-intro-frame">
        <div className="page-intro-copy">
          <h1 className={headingLanguageClass(t(title))}><InlineMarkup text={title} /></h1>
        ...
        {(image || mobileImage) && <div className="page-intro-media">
          {video ? <video src={image || undefined} poster={mobileImage || undefined} autoPlay muted loop playsInline preload="metadata" aria-label={t(imageAlt)} /> : <picture>
            {mobileImage && <source media="(max-width: 639px)" srcSet={mobileImage} />}
            <img src={image || mobileImage || ''} alt={t(imageAlt)} fetchPriority="high" decoding="async" />
          </picture>}
        </div>}
      </div>
    </section>
  ...
  return (
    <section className={`overflow-hidden bg-dark ${className}`}>
      <div className={`relative isolate flex w-full items-center overflow-hidden bg-dark ${height}`}>
        {video && image ? (
          <>
            {mobileImage && <img src={mobileImage} alt={t(imageAlt)} loading="eager" fetchPriority="high" decoding="async" className={`${mediaClass} sm:hidden`} />}
            <video src={image} autoPlay muted loop playsInline preload="auto" aria-label={t(imageAlt)} className={`absolute inset-0 -z-20 h-full w-full object-cover object-center ${home ? 'motion-hero-media' : ''} ${mobileImage ? 'hidden sm:block' : ''}`} />
          </>
        ) : image || mobileImage ? (
          <picture>
            {mobileImage && <source media="(max-width: 639px)" srcSet={mobileImage} />}
            <img src={image || mobileImage || ''} alt={t(imageAlt)} loading="eager" fetchPriority="high" decoding="async" className={mediaClass} />
          </picture>
        ) : (
          <div className="gonyik-material-placeholder absolute inset-0 -z-20" role="img" aria-label={t(imageAlt || '材料结构示意')} />
        )}
        ...
        <div className={`mx-auto w-full ${home ? 'home-hero-frame max-w-[1264px] px-5 md:px-8' : 'max-w-[1760px]'}`}>
          <div className={`w-full text-white ${home ? 'home-hero-copy max-w-[920px] pb-32 pt-16 md:pb-36' : 'max-w-[720px] px-7 py-12 md:px-12 lg:px-16'}`}>
            ...
            <h1 style={home ? { '--motion-delay': '140ms' } as CSSProperties : undefined} className={`whitespace-pre-line ${headingLanguageClass(t(title))} ${home ? 'motion-hero-item type-hero leading-[1.12] tracking-[-.02em]' : 'type-page-title'}`}><InlineMarkup text={title} /></h1>
