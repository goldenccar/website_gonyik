...
export default function HorizontalRail({ children, label, mobileStack = false }: { children: ReactNode; label: string; mobileStack?: boolean }) {
  ...
  return <div>
    <div ref={railRef} onScroll={updateProgress} aria-label={label} className={`gonyik-rail grid ${layout}`}>{children}</div>
    <ReadingProgress progress={progress} className={`mt-[-22px] ${mobileStack ? 'hidden sm:block' : 'block'}`} />
  </div>
}
