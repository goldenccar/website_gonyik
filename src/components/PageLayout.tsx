...
export function SectionHeader({ tag, title, subtitle, linkText, linkTo, light = false }: {
  ...
  return (
    <div className="mb-10 flex flex-col gap-5 md:mb-12 md:flex-row md:items-end md:justify-between">
      ...
      {linkText && linkTo && <Link to={localePath(linkTo)} className={`text-[14px] font-medium underline underline-offset-4 ${light ? 'text-white' : 'text-primary'}`}><InlineMarkup text={linkText} /> →</Link>}
    </div>
  )
}
