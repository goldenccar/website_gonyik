...
export default function ApplicationCard({ product, categoryName }: { product: EquipmentProduct; categoryName?: string }) {
  return <article className="group min-w-0 snap-start">
    ...
    <p className="mt-3 text-label uppercase text-secondary"><InlineMarkup text={categoryName} /></p>
    <h3 className="mt-1 text-h5 text-primary"><InlineMarkup text={product.name} /></h3>
    <p className="mt-2 text-[14px] text-secondary"><InlineMarkup text={product.card_summary || parseFeatures(product.features).slice(0, 3).join(' · ')} /></p>
  </article>
}
