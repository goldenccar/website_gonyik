...
- 系列首次出现时只显示系列说明和 SKU 卡片，不默认展开 OT-01 或任何 SKU 的性能，也不显示“选择一个面料型号”等占位提示；
- 点击整张 SKU 卡或“了解详情”后，在该系列 SKU 横向轨道正下方展开一个 `SkuDetailPanel`，不是全局弹窗；
- 同一系列一次只展开一个 SKU，切换 OT-01/OT-02 时复用同一个详情面板并替换内容；
- 再次点击当前 SKU、点击关闭或选择其他系列时收起详情；
- 面板标题必须明确显示当前型号，例如“Otter OT-01”；
- 性能、结构、报告、规格和样品 CTA 全部读取当前选中 SKU，严禁沿用上一个 SKU 数据；
- OT-01 的实测结果只在 OT-01 详情中出现，不适用于 OT-02 或 Otter 系列整体；
...
- 官网、样卡、报价单和客户沟通只显示对外简码，格式为 `OT-01`、`RA-01`、`KA-01`；
- 内部结构码仅用于研发、打样、生产、质检和内部技术资料，不进入公共网页数据；
- `OT-01` 的过渡名称可写为 `OT-01（原T31）`，不继续使用 `T31` 作为正式代码；
- 显示所属系列；
...
| OT-01 | SKU 卡主标题和对外简码 |
| OT3-PAEL70-V15-PES50-B | 仅后台和内部技术资料使用 |
| T31 / GY-OTTER-T31 / OT-001 | 删除；过渡文案可写“OT-01（原T31）” |
...
```ts
sku_code: string // 对外简码：OT-01 / RA-01 / KA-01
internal_code: string // 内部结构码
development_status: DevelopmentStatus
visibility: Visibility
...
- Otter = `OT`，Rayo = `RA`，Kais = `KA`；
- 当前正式对应仅有 `OT-01 ↔ OT3-PAEL70-V15-PES50-B` 和 `OT-02 ↔ OT3-PAEL50-V20-PES30-D`；
- Rayo 与 Kais 只保留编码规则，在真实结构确认前不创建 `RA-01` 或 `KA-01`；
- 不得根据 `Kinetic`、`Lumix` 等旧名称自动生成新代码；
- 克重、胶量、纤维比例、门幅、颜色、测试结果、认证、批次和开发状态不进入产品代码；
- 已使用的对外简码和内部结构码必须一一对应，不得重复分配；主体材料、旦数、膜版本、核心层、底层或实质性复合结构变化时建立新产品号。
...
- 面料数据库初始状态不显示 OT-01 或任何 SKU 的性能；
...
- 选择 OT-01 后只显示 OT-01 的结构、性能和报告，切换 OT-02 后不会残留 OT-01 数据；
